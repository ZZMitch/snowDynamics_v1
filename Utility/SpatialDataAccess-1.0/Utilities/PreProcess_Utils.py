...
def downloadNC(cube, path, file, form = ''):
    ...
    cube.to_netcdf(path = os.path.join(path, file),
                   mode = 'w', # Overwrite files 
                   format = 'NETCDF4', # HDF5 file with netCDF4 API  
                   engine = 'netcdf4')
######################################################################################################################################################
...
def sameDayMerge(cube, form, con, bands, sat, verbose = True):
    ...
    if sat == 'hls':
        cube = cube.drop_vars(['id', 'end_datetime', 'eo:cloud_cover', 'start_datetime', 'epsg'], errors = 'ignore')
    if sat == 's2':

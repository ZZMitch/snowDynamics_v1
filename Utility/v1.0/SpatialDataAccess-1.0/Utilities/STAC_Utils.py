...
# Get monthly clear HLS observation counts from point data.
def sampleMonthlyClearCountsHLS(pts, start, end, sceneCloudThresh, snowClear, verbose): # Will need upddate based on changes to rescale/pixelMask...

    """
    Parameters:
    pts (string): path to point shp
    ...
    snowClear (boolean): Whether (True) or not (False) snow should be considered clear.

    Returns:
    CSVs with monthly clear observations counts from L30 and S30
    """
    ...
    # Prepare or load dataframes
    if os.path.exists('MonthlyClear_Landsat.csv'): 

        # L30
        df_L = pd.read_csv('MonthlyClear_Landsat.csv')
        df_L['month_ld'] = pd.to_datetime(df_L['month_ld'])

        # S30
        df_S = pd.read_csv('MonthlyClear_Sentinel2.csv')
        df_S['month_ld'] = pd.to_datetime(df_S['month_ld'])
        
        if verbose == True:
            print('Loaded pre-existing CSVs with', df_L.shape[1] - 2, 'completed points.')
    
    else: # Create blank dataframes to fill

        df = pd.DataFrame(columns = ['month', 'month_ld'])

        # Get list of months
        months = [dt for dt in rrule(MONTHLY, dtstart = dt.datetime.strptime(start, '%Y-%m-%d'), until = dt.datetime.strptime(end, '%Y-%m-%d'))]
        df['month'] = [month.strftime('%Y-%m') for month in months]

        # Last of day of the month to match xarray outputs
        df['month_ld'] = pd.to_datetime(df['month'])

        for date in range(len(df)):
            res = cal.monthrange(df['month_ld'][date].year, df['month_ld'][date].month)
            day = res[1]
            df.loc[date, 'month_ld'] = str(df.loc[date, 'month_ld'].year) + '-' + str(df.loc[date, 'month_ld'].month) + '-' + str(day)

        df_L = df.copy()
        df_S = df.copy()

        if verbose == True: 
            print('Created blank dataframes with', len(df), 'months to fill.')

    # Access NASA Earthdata STAC
    gdalEnv, catalog = accessSTAC('https://cmr.earthdata.nasa.gov/stac/LPCLOUD')

    if verbose == True:
        print('Entered NASA Earthdata LPCLOUD Catalog.')

# Cycle through all points and get monthly clear observation counts
    for pt in pts.index: # For each point...

        try: # Getting around server issues every so-often...

            if str(pt) not in df_L.columns: # If point not already analyzed...

                if verbose == True:
                    print('--------------------')
                    print('Starting point', pt)

                # Create BBox with tiny edge size (isolate single pixel)
                bboxUTM, bboxLL, epsg = pputil.latlon2bboxes(pts['lat'][pt], pts['lon'][pt], 0.01)

                if verbose == True:
                    print('Bounding box created.')   

                # Get all L30/S30 items over BBox # 'HLSL30.v2.0', 'HLSS30.v2.0'
                itemsL30 = catalog.search(intersects = bboxLL, datetime = f'{start}/{end}', collections = ['HLSL30_2.0'], 
                                          limit = 100).item_collection()
                itemsS30 = catalog.search(intersects = bboxLL, datetime = f'{start}/{end}', collections = ['HLSS30_2.0'], 
                                          limit = 100).item_collection()
                
                if verbose == True:
                    print('STAC Items found for L30 and S30 (n =' + str(len(itemsL30)), ' + ', str(len(itemsS30)) + ').')

                # Build L30/S30 stacks
                with warn.catch_warnings():
                    warn.simplefilter('ignore') # Avoids infer date time depreciation warnings from stackstac

                    l30 = ss.stack(itemsL30, 
                                   assets = ['Fmask'], 
                                   resolution = 30, 
                                   bounds = bboxUTM, 
                                   epsg = epsg, 
                    ...
                    s30 = ss.stack(itemsS30, 
                                   ...
                                   gdal_env = gdalEnv)
                if verbose == True:
                    print('Image stacks built.')

                # If multiple pixels get pulled, pick the first one
                if l30.shape[2] > 1 or l30.shape[3] > 1:
                    l30 = l30[:,:,0,0]

                if s30.shape[2] > 1 or s30.shape[3] > 1:
                    s30 = s30[:,:,0,0]

                # Remove bad image time-steps (NaT, high scene-level cloud cover)
                l30 = pputil.removeBadScenes(l30, sceneCloudThresh, False)
                s30 = pputil.removeBadScenes(s30, sceneCloudThresh, False)

                if verbose == True:
                    print('Removed images above ' + str(sceneCloudThresh) + '% scene-level clouds (n =' + 
                          str(l30.shape[0]), ' + ', str(s30.shape[0]), ').')

                # Load image stacks into memory
                print('Loading image stacks into memory...')
                l30 = pputil.loadXR(l30)
                s30 = pputil.loadXR(s30)       

                # Remove NA time-steps
                l30 = pputil.rescaleReduce(l30, 1, 255, verbose = False).squeeze() # Single pixel so NA threshold does not matter (just above 0)
                s30 = pputil.rescaleReduce(s30, 1, 255, verbose = False).squeeze()  

                # Convert to DataFrame
                l30 = l30.to_pandas().to_frame(name = 'Fmask')
                s30 = s30.to_pandas().to_frame(name = 'Fmask')  

                # Remove duplicate observations (not using regular merge function because it takes ~20 secs even with tiny DataArray)
                l30 = l30.groupby(by = l30.index.date).max()
                s30 = s30.groupby(by = s30.index.date).max()

                if verbose == True:
                    print('Same-day observations merged ( n =', len(l30), '+', len(s30), ').')

                # Unpack Fmask and get clear observations
                l30['Fmask'] = pputil.convertFmask(l30['Fmask'], format = 'clearUnclear', snowClear = snowClear)
                s30['Fmask'] = pputil.convertFmask(s30['Fmask'], format = 'clearUnclear', snowClear = snowClear)

                if verbose == True:
                    print('Clear observations identified ( n =', l30['Fmask'].sum(), '+', s30['Fmask'].sum(), ').')

                # Resample to monthly sums (remember that 1 = clear and 0 = unclear)
                l30.index = pd.to_datetime(l30.index)
                l30 = l30.resample('ME').sum()
                
                s30.index = pd.to_datetime(s30.index)
                s30 = s30.resample('ME').sum()

                if verbose == True:
                    print('Monthly clear sums calculated ( max =', l30['Fmask'].max(), ',', s30['Fmask'].max(), ').')

                # Create pt dataframes to hold monthly sums
                pt_df_L = pd.DataFrame()
                pt_df_L['month_ld'] = l30.index
                pt_df_L[str(pts.index[pt])] = l30.values

                pt_df_S = pd.DataFrame()
                pt_df_S['month_ld'] = s30.index
                pt_df_S[str(pts.index[pt])] = s30.values   

                # Merge with master dataframes
                df_L = df_L.merge(pt_df_L, how = 'outer')
                df_L[str(pts.index[pt])] = df_L[str(pts.index[pt])].fillna(0).astype(int)

                df_S = df_S.merge(pt_df_S, how = 'outer')
                df_S[str(pts.index[pt])] = df_S[str(pts.index[pt])].fillna(0).astype(int)

                if verbose == True:
                    print('Monthly clear sums merged with master dataframes.')

                # Export CSVs
                df_L.to_csv('MonthlyClear_Landsat.csv', index = False, mode = 'w+') # Overwrite
                df_S.to_csv('MonthlyClear_Sentinel2.csv', index = False, mode = 'w+')
                
                print('Master dataframes exported to csv.')
                print('--------------------')
        except Exception as error: # Server errors every so often...
            print('Skipping point', pt, '(', type(error).__name__, ').')
            continue                                                                       
...
def observationAvailabilityHLS(area_shp, 
    ...
    download (str): Column name in area indicating which polygons to download fmask data from. 1 = download, 0 = do not download.
    - Currently downloads as filesystem of GeoTiffs, but support for NetCDF or zarr is possible. 
    ...
    if verbose == True:
        print('Loaded CSVs with', ((obs_fill_S30.null_count() < len(obs_fill_S30)).sum_horizontal() - 1).item(), 'completed polygons.')
    ...
    start = str(year) + '-01-01' # For 2017 only - use August 1 to Dec 31 (snow period monitoring)
    end = str(year) + '-12-31'
            ...
            if area.loc[poly, progressTrack] != 1:
                ...
                if metadata == True: # Record info in Metadata
                    ...
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_items_L30')
                                               .then(nL30) # Number of L30 items found on STAC
                                               .otherwise(pl.col(str(poly)))
                                               .alias(str(poly)))           
                ...
                if metadata == True: # Record info in Metadata
                    ...
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_items_S30')
                                               .then(nS30) # Number of S30 items found on STAC
                                               .otherwise(pl.col(str(poly)))
                                               .alias(str(poly)))   
                ...
                if (nL30 == 0) & (metadata == True):
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_stack_L30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))    
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_remove_L30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))  
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_lazy_L30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_L30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'gb_merge_L30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))               
                ...
                if (nS30 == 0) & (metadata == True):
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_stack_S30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))    
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_remove_S30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))  
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_lazy_S30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_S30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'gb_merge_S30').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))   
                ...
                if metadata == True:
                    ...
                    if nL30 > 0: 
                        l30_90 = l30_remove.sel(time = l30_remove['eo:cloud_cover'].values <= 90)
                        l30_90 = l30_90['time'] = l30_90['time'].dt.floor('1D')
                        l30_75 = l30_remove.sel(time = l30_remove['eo:cloud_cover'].values <= 75)
                        l30_75 = l30_75['time'] = l30_75['time'].dt.floor('1D')
                        l30_50 = l30_remove.sel(time = l30_remove['eo:cloud_cover'].values <= 50)
                        l30_50 = l30_50['time'] = l30_50['time'].dt.floor('1D')
                        l30_10 = l30_remove.sel(time = l30_remove['eo:cloud_cover'].values <= 10)
                        ...
                        if len(l30_10) == 0:
                            meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_L30_10')
                                                    .then(0) # Number of time-steps in merged L30 cube (10+% cloud filter)
                                                    .otherwise(pl.col(str(poly)))
                                                    .alias(str(poly)))  
                    if nL30 == 0:
                    ...
                    if nS30 > 0:                        
                        s30_90 = s30_remove.sel(time = s30_remove['eo:cloud_cover'].values <= 90)
                        s30_90 = s30_90['time'] = s30_90['time'].dt.floor('1D')
                        s30_75 = s30_remove.sel(time = s30_remove['eo:cloud_cover'].values <= 75)
                        s30_75 = s30_75['time'] = s30_75['time'].dt.floor('1D')
                        s30_50 = s30_remove.sel(time = s30_remove['eo:cloud_cover'].values <= 50)
                        s30_50 = s30_50['time'] = s30_50['time'].dt.floor('1D')
                        s30_10 = s30_remove.sel(time = s30_remove['eo:cloud_cover'].values <= 10)
                        ...
                        if len(s30_90) == 0:
                            meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_S30_90')
                                                    .then(0) # Number of time-steps in merged S30 cube (90+% cloud filter)
                                                    .otherwise(pl.col(str(poly)))
                                                    .alias(str(poly))) 
                        ...
                        if len(s30_75) == 0:
                            meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_S30_75')
                                                    .then(0) # Number of time-steps in merged S30 cube (75+% cloud filter)
                                                    .otherwise(pl.col(str(poly)))
                                                    .alias(str(poly)))                   
                        ...
                        if len(s30_50) == 0:
                            meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_S30_50')
                                                    .then(0) # Number of time-steps in merged S30 cube (50+% cloud filter)
                                                    .otherwise(pl.col(str(poly)))
                                                    .alias(str(poly)))                            
                        ...
                        if len(s30_10) == 0:
                            meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_merge_S30_10')
                                                    .then(0) # Number of time-steps in merged S30 cube (10+% cloud filter)
                                                    .otherwise(pl.col(str(poly)))
                                                    .alias(str(poly)))     
                ...
                if metadata == True:
                    ...
                    if (nL30 > 0) | (nS30 > 0): 
                        ...
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_obs_load')
                                                .then(fmask.shape[0] * fmask.shape[-2] * fmask.shape[-1]) # Number of total observations in combined cube
                                                .otherwise(pl.col(str(poly)))
                                                .alias(str(poly)))
                        ...
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_load')
                                                ...
                                                .alias(str(poly)))
                    if (nL30 == 0) & (nS30 == 0):
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_load').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'nunique_load').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_obs_load').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'gb_load').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_load').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))                        
                ...
                if (nL30 > 0) | (nS30 > 0): 
                    ...
                    if download != '':
                        ...
                        if area.loc[poly, download] == 1: # Only download download polys
                            ...
                            pputil.downloadNC(fmask, downloadPath, 'fmask_' + str(poly) + '_' + str(year) + '.nc', form = 'hls')
                ...
                if (metadata == True) & (nL30 == 0) & (nS30 == 0):
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_convert').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))                    
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_mask').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    if borderCleaning != '': 
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 'percentage')
                                                .then(area.loc[poly, borderCleaning])
                                                .otherwise(pl.col(str(poly)))
                                                .alias(str(poly)))      
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 'n_obs_ts').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))                     
                    meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_counts').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))
                    if download != '':
                        meta = meta.with_columns(pl.when(pl.col('Metadata') == 't_download').then(0).otherwise(pl.col(str(poly))).alias(str(poly)))                                                        

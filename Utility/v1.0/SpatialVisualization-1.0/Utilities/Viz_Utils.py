...
def timeSeriesPlot(df, df_col = '', form = '', save = ''):
    ...
    if form == 'ims':
        ...
        for i in range(len(dates) - 1): # For the all date indexes except the last one
            ...
            # Winter year snow maximum (using smin to find winter year)
            # Filter to winter year
            smax = df_sm.filter(pl.col('time').is_between(dt.date(dates[i].year, dates[i].month, dates[i].day),
                                                          dt.date(dates[i + 1].year, dates[i + 1].month, dates[i + 1].day),
                                                          closed = 'right')) # Day after minimum to minimum
            #print(smax[0]['time'][0]) # Winter year start date
            #print(smax[-1]['time'][0]) # Winter year end date

            # Find all time-steps with maximum value and get median date
            smax = smax.with_columns(pl.when(pl.col(df_col) == smax.max()[df_col]).then(smax.max()[df_col]).name.keep()).drop_nulls().median()

            # Plot marker at smoothed location
            plt.plot(smax['time'], smax[df_col], color = 'blue', markeredgecolor = 'black', marker = 'o', markersize = markersize)

            # Annotate marker with date
            if smax[df_col][0] < 97: # Far enough from top, put above
                ax.annotate(smax['time'][0].strftime('%m/%d'), xy = (mdates.date2num(smax['time']), smax[df_col][0]), xytext = (-14, y_txt), 
                            textcoords = 'offset points', path_effects = [pe.withStroke(linewidth = linewidth, foreground = 'blue')], 
                            fontsize = fontsize)
            if smax[df_col][0] >= 97: # Close to top, put below
                ax.annotate(smax['time'][0].strftime('%m/%d'), xy = (mdates.date2num(smax['time']), smax[df_col][0]), xytext = (-14,-y_txt-7), 
                            textcoords = 'offset points', path_effects = [pe.withStroke(linewidth = linewidth, foreground = 'blue')], 
                            fontsize = fontsize)
...
def ObsPredPlot(pred, obs, weights = [], quality = [], q_range = [], pred_label = '', obs_label = '', q_label = 'Quality', alpha = 0.5, w_size = 50): 
    ...
    if len(weights) == 0:
        r2 = r2_score(obs, pred) # In this scenario x is y_pred... Full eq: 1 - (((y - x) ** 2).sum() / ((y - y.mean()) ** 2).sum())
    ...
    if len(weights) > 0:
        r2 = r2_score(obs, pred, sample_weight = weights)
...
def APUcurves(pred, obs, weights = [], quality = [], q_range = [], label = '', q_label = 'Quality', alpha = 0.5, w_size = 50):
    ...
    Returns:
    APU curves + scatter plot. Y axis is residuals (x - y), X axis is y (observed values).
    """
